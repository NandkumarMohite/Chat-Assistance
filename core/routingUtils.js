...
function normalizeQueryParams(apiDef, queryParams = {}, dateAliases = DEFAULT_DATE_ALIASES) {
  ...
  // Handle stationId <-> stationIds <-> stations normalization
  // stationId = single integer (one station)
  // stationIds / stations = comma-separated string (e.g. "1,2,3")
  const toCommaSeparated = (val) => {
    if (Array.isArray(val)) return val.join(',');
    return String(val);
  };
  const toSingleId = (val) => {
    if (Array.isArray(val)) return val[0];
    if (typeof val === 'string' && val.includes(',')) return parseInt(val.split(',')[0], 10);
    return typeof val === 'string' ? parseInt(val, 10) : val;
  };

  if (normalized.stationId !== undefined && !apiParamNames.includes('stationId')) {
    if (apiParamNames.includes('stationIds')) {
      normalized.stationIds = toCommaSeparated(normalized.stationId);
      delete normalized.stationId;
    } else if (apiParamNames.includes('stations')) {
      normalized.stations = toCommaSeparated(normalized.stationId);
      delete normalized.stationId;
    }
  }

  if (normalized.stationIds !== undefined && !apiParamNames.includes('stationIds')) {
    if (apiParamNames.includes('stationId')) {
      normalized.stationId = toSingleId(normalized.stationIds);
      delete normalized.stationIds;
    } else if (apiParamNames.includes('stations')) {
      normalized.stations = toCommaSeparated(normalized.stationIds);
      delete normalized.stationIds;
    }
  }

  if (normalized.stations !== undefined && !apiParamNames.includes('stations')) {
    if (apiParamNames.includes('stationIds')) {
      normalized.stationIds = toCommaSeparated(normalized.stations);
      delete normalized.stations;
    } else if (apiParamNames.includes('stationId')) {
      normalized.stationId = toSingleId(normalized.stations);
      delete normalized.stations;
    }
  }

  // Ensure stationIds/stations are always comma-separated strings (not arrays)
  if (normalized.stationIds !== undefined && Array.isArray(normalized.stationIds)) {
    normalized.stationIds = normalized.stationIds.join(',');
  }
  if (normalized.stations !== undefined && Array.isArray(normalized.stations)) {
    normalized.stations = normalized.stations.join(',');
  }

  // Handle model <-> models normalization
  // model = single string (one model)
  // models = comma-separated string (e.g. "MODEL_A,MODEL_B")
  if (normalized.model !== undefined && !apiParamNames.includes('model') && apiParamNames.includes('models')) {
    normalized.models = toCommaSeparated(normalized.model);
    delete normalized.model;
  } else if (normalized.models !== undefined && !apiParamNames.includes('models') && apiParamNames.includes('model')) {
    const val = normalized.models;
    normalized.model = Array.isArray(val) ? val[0] : (typeof val === 'string' && val.includes(',') ? val.split(',')[0] : val);
    delete normalized.models;
  }

  // Ensure models is always a comma-separated string (not array)
  if (normalized.models !== undefined && Array.isArray(normalized.models)) {
    normalized.models = normalized.models.join(',');
  }
