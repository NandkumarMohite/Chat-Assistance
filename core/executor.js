...
const { resolveTemporalQueryParams } = require('./dateRange');
const { TEST_LOCALLY } = require('./config');
const { getMockResponse } = require('./test-mock-data');
...
async function executeDependencyChain(chain, userMessage, jwtToken, send, timeZone = 'UTC') {
  ...
  for (let i = 0; i < chain.steps.length; i++) {
    ...
    // Build URL with path + optional query params
    const resolvedQueryParams = {};
    if (step.queryParams) {
      for (const [key, val] of Object.entries(step.queryParams)) {
        resolvedQueryParams[key] = typeof val === 'string' && val.startsWith('$')
          ? context[val.slice(1)]
          : val;
      }
    }
    const normalizedQueryParams = resolveTemporalQueryParams(resolvedQueryParams, timeZone);
    const fullUrl = buildApiUrl(registry.baseUrl, apiDef.path, resolvedPathParams, normalizedQueryParams);

    send('api_call', { method: apiDef.method, url: fullUrl, body: null, name: apiDef.name });
    send('status', { step: `chain_${i}`, message: TEST_LOCALLY ? `🧪 [MOCK] Step ${i + 1}/${chain.steps.length}: Loading ${apiDef.name}...` : `🔗 Step ${i + 1}/${chain.steps.length}: Calling ${apiDef.name}...` });

    let result;
    if (TEST_LOCALLY) {
      result = getMockResponse(step.apiId);
      console.log(`[TEST_LOCALLY] Using mock data for chain step: ${step.apiId}`);
    } else {
      result = await executeAPICall({ method: apiDef.method, url: fullUrl, body: null }, jwtToken);
    }
